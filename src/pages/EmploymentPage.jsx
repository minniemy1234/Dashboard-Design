...
import {
  ...
  ResponsiveContainer,
  Cell,
  LabelList // 🛠️ เพิ่ม Component สำหรับแสดงตัวเลขบนแท่งกราฟ
} from "recharts";
...
function EmploymentPage() {
  ...
  const processedData = useMemo(() => {
    ...
    return validRows.map(item => {
      ...
      const q2 = getVal("ผู้สำเร็จการศึกษา"); 
      const q5 = getVal("มีงานทำเดิม");
      const q6 = getVal("ศึกษาต่อ");
      const q7 = getVal("บัณฑิตบวช");
      const q8 = getVal("บัณฑิตเกณฑ์ทหาร");
      
      const gov = getVal("ทำงานในหน่วยงานรัฐจำนวน");
      const state = getVal("ทำงานในหน่วยงานรัฐวิสาหกิจจำนวน");
      const privateOrg = getVal("ทำงานในหน่วยงานเอกชนจำนวน");
      const inter = getVal("ทำงานในองค์การต่างประเทศ/ระหว่างประเทศจำนวน");
      const otherOrg = getVal("ทำงานในองค์กรอื่นๆจำนวน");
      const q3 = gov + state + privateOrg + inter + otherOrg;

      const q4 = getVal("ทำงานธุรกิจส่วนตัว/อิสระจำนวน");
      const q9 = 0; 
  ...
  const tableColumns = [
    ...
    { title: "ผู้สำเร็จการศึกษา (คน)", dataIndex: "q2", key: "q2", align: "center", render: v => v.toLocaleString() },
    { title: "ได้งานประจำ (คน)", dataIndex: "q3", key: "q3", align: "center", render: v => v.toLocaleString() },
    { title: "อาชีพอิสระ (คน)", dataIndex: "q4", key: "q4", align: "center", render: v => v.toLocaleString() },
    { title: "ศึกษาต่อ (คน)", dataIndex: "q6", key: "q6", align: "center", render: v => v.toLocaleString() },
    { title: "หักลบอื่นๆ (5+7+8)", key: "others", align: "center", render: (_, r) => (r.q5 + r.q7 + r.q8).toLocaleString() },
    { 
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 20, marginBottom: 24 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 15 }}>
              <div style={{ background: "#e6f7ff", padding: 20, borderRadius: 15, border: "1px solid #91d5ff" }}>
                ...
                <h4 style={{ margin: "5px 0", fontSize: 13, color: "#0050b3" }}>ผู้สำเร็จการศึกษา</h4>
                <h2>{totals.q2.toLocaleString()} <span style={{ fontSize: 13, fontWeight: 'normal', color: '#8c8c8c' }}>คน</span></h2>
              ...
              <div style={{ background: "#e6f7ff", padding: 20, borderRadius: 15, border: "1px solid #91d5ff" }}>
                ...
                <h2>{totals.q3.toLocaleString()} <span style={{ fontSize: 13, fontWeight: 'normal', color: '#8c8c8c' }}>คน</span></h2>
              </div>

              <div style={{ background: "#e6f7ff", padding: 20, borderRadius: 15, border: "1px solid #91d5ff" }}>
                <div style={{ color: "#8c8c8c", fontSize: 11 }}>สถานะการทำงาน</div>
                <h4 style={{ margin: "5px 0", fontSize: 13, color: "#0050b3" }}>ประกอบอาชีพอิสระ</h4>
                <h2>{totals.q4.toLocaleString()} <span style={{ fontSize: 13, fontWeight: 'normal', color: '#8c8c8c' }}>คน</span></h2>
              </div>
              ...
              <div style={{ background: "#f8fafc", padding: 16, borderRadius: 12, border: "1px solid #e2e8f0", gridColumn: "span 3", display: "flex", justifyContent: "space-around", alignItems: "center" }}>
                <div style={{ textAlign: "center" }}>
                  ...
                  <strong style={{ fontSize: 16, color: "#334155" }}>{totals.q6.toLocaleString()} คน</strong>
                </div>
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 11, color: "#64748b" }}>มีงานทำเดิม</div>
                  <strong style={{ fontSize: 16, color: "#334155" }}>{totals.q5.toLocaleString()} คน</strong>
                </div>
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 11, color: "#64748b" }}>บวชเรียน</div>
                  <strong style={{ fontSize: 16, color: "#334155" }}>{totals.q7.toLocaleString()} คน</strong>
                </div>
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 11, color: "#64748b" }}>เกณฑ์ทหาร</div>
                  <strong style={{ fontSize: 16, color: "#334155" }}>{totals.q8.toLocaleString()} คน</strong>
                </div>
              </div>
            </div>
          </div>
            ...
            <div style={{ height: 400 }}>
              {dynamicChartData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  {/* เพิ่มขอบขวาของ Grid ขยับเพิ่มจาก 40 เป็น 65 เพื่อกันไม่ให้ตัวเลขหลักร้อยหลุดขอบขวาตัวกราฟ */}
                  <BarChart data={dynamicChartData} layout="vertical" margin={{ top: 10, right: 65, left: 160, bottom: 10 }}>
                    ...
                    <Bar dataKey="จำนวนคน" radius={[0, 6, 6, 0]} barSize={24}>
                      {/* 🛠️ เพิ่ม LabelList เพื่อให้พ่นตัวเลขจำนวนคนออกมาต่อท้ายแท่งทันที */}
                      <LabelList 
                        dataKey="จำนวนคน" 
                        position="right" // สั่งให้อยู่ขวาถัดออกมาจากปลายแท่ง
                        dx={8}           // ปรับระยะห่างจากปลายแท่งเล็กน้อยเพื่อความสวยงาม
                        style={{ fill: '#475569', fontSize: 12, fontWeight: 'bold' }} 
                        formatter={(value) => `${value.toLocaleString()} คน`} // แสดงค่าพร้อมสรุปหน่วยท้ายตัวเลข
                      />
